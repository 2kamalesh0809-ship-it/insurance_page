document.addEventListener('DOMContentLoaded', () => {
    ...
    if (partnersTrack) {
        ...
        function getVisibleSlides() {
            if (window.innerWidth > 1024) return 4; // Compact on desktop
        ...
        function updateCarousel() {
            ...
            const slideWidth = partnerSlides[0].getBoundingClientRect().width;

            if (isDesktop) {
                // Scroll the track which has overflow-x: auto
                partnersTrack.scrollTo({
                    left: currentSlide * (slideWidth + 20),
                    behavior: 'smooth'
                });
            } else {
                partnersTrack.style.transform = `translateX(-${currentSlide * slideWidth}px)`;
            }
        }
        ...
        function nextSlide() {
            ...
            if (currentSlide >= total - (window.innerWidth > 1024 ? 1 : visible)) {
                currentSlide = 0;
            } else {
                currentSlide++;
            }
        ...
        const container = partnersTrack.parentElement;
        ...
        window.addEventListener('resize', () => {
            ...
            if (window.innerWidth > 1024) {
                partnersTrack.style.transform = 'none';
            }
            updateCarousel();
        });
